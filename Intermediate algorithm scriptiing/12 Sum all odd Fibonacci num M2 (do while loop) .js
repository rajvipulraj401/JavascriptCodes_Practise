// 12 Sum All Odd Fibonacci Numbers (Method 2 with do-while loop)
...
function sumFibs(num) {
  ...
  // Use a do-while loop to ensure the loop runs at least once, even if num is less than 0.
  do {
    if (a % 2 === 1) {
      sum += a;
    }
...
3) Use a do-while loop to ensure the loop runs at least once, even if num is less than 0. Inside the loop, check if a is less than or equal to the given num. Inside the loop, use an if statement to check if a is an odd number. If it is, increment the sum by the odd number.
